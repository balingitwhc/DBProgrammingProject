...
namespace InventorySystemProject
{
    public partial class InventoryLogs : Form
    {
        ...
        private void frmInventoryLogs_Load(object sender, EventArgs e)
        {
        ...
        private void LoadInventoryLogDetails()
        {
            ...
            {
                DataRow selectedInventoryLog = ds.Tables[0].Rows[0];

                txtProductName.Text = selectedInventoryLog["ProductName"].ToString();
                ...
                firstInventoryLogId = Convert.ToInt32(ds.Tables[1].Rows[0]["FirstInventoryLogId"]);
                previousInventoryLogId = ds.Tables[1].Rows[0]["PreviousInventoryLogId"] != DBNull.Value ? Convert.ToInt32(ds.Tables["Table1"].Rows[0]["PreviousInventoryLogId"]) : (int?)null;
                nextInventoryLogId = ds.Tables[1].Rows[0]["NextInventoryLogId"] != DBNull.Value ? Convert.ToInt32(ds.Tables["Table1"].Rows[0]["NextInventoryLogId"]) : (int?)null;
                lastInventoryLogId = Convert.ToInt32(ds.Tables[1].Rows[0]["LastInventoryLogId"]);
                logCount = Convert.ToInt32(ds.Tables[1].Rows[0]["LogCount"]);
